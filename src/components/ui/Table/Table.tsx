...
const Table = <T, K extends keyof T>({
  ...
  return (
    <div
      className={classNames('overflow-hidden shadow-sm', className, {
        'border border-gray-200 dark:border-gray-700 rounded-lg': bordered,
      })}
    >
      <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
        {showHeader && (
          <thead data-testid="table-header" className="bg-gray-50 dark:bg-gray-700">
            <tr>
        ...
        <tbody
          className={`divide-y divide-gray-200 dark:divide-gray-700 ${striped ? 'bg-white dark:bg-gray-800' : ''}`}
        >
          {data.length > 0 ? (
